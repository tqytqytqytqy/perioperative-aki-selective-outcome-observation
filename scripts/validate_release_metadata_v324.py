#!/usr/bin/env python3
"""Validate the journal-neutral v3.2.4 release metadata and public boundary."""
...
VERSION_DOI = "10.5281/zenodo.21663368"
...
def main() -> int:
    ...
    add(
        ...
        any(
            item.get("identifier", "").endswith("/tree/v3.2.4")
    ...
    add(
        checks,
        "citation_version",
        bool(citation_version and citation_version.group(1) == "3.2.4"),
    ...
    add(
        checks,
        "citation_version_doi",
        bool(citation_doi and citation_doi.group(1) == VERSION_DOI),
        citation_doi.group(1) if citation_doi else None,
    )
    add(
        checks,
        "citation_version_doi_url",
        bool(citation_url and citation_url.group(1) == f"https://doi.org/{VERSION_DOI}"),
    ...
    add(checks, "readme_version", readme.startswith("# Selective outcome observation study v3.2.4"), readme.splitlines()[0])
    add(checks, "readme_version_doi", VERSION_DOI in readme, VERSION_DOI)
    add(checks, "model_card_version", model_card.startswith("# Model card v3.2.4"), model_card.splitlines()[0])
    add(checks, "model_card_version_doi", VERSION_DOI in model_card, VERSION_DOI)
    ...
    stale_versions = [
        value
        for value in ("10.5281/zenodo.21447516",)
    ...
    add(
        ...
        and str(citation_version.group(1) if citation_version else "") == "3.2.4",
        {
            "hits": stale_versions,
    ...
    add(
        ...
        == "Selective outcome observation study v3.2.4",
        workbook["README"]["A1"].value,
    )
    workbook.close()
    ...
    output_path = ROOT / "qa" / "release_metadata_checks_v324.json"
