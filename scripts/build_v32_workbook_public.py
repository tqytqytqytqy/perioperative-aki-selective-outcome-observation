...
def public_disclosure_frame(filename: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Apply disclosure controls to public displays without altering analysis sources."""
    public = frame.copy()
    if filename == "33_predictor_missingness_v32.csv":
        missing_n = pd.to_numeric(public["missing_n"], errors="coerce")
        small = missing_n.gt(0) & missing_n.lt(SMALL_CELL_THRESHOLD)
        public["missing_n"] = public["missing_n"].astype(object)
        public["missing_percent"] = public["missing_percent"].astype(object)
        for index in public.index[small]:
            percent_limit = SMALL_CELL_THRESHOLD / float(public.at[index, "eligible_n"]) * 100
            public.at[index, "missing_n"] = f"<{SMALL_CELL_THRESHOLD}"
            public.at[index, "missing_percent"] = f"<{percent_limit:.2f}"
    return public
...
def main() -> int:
    tables = {
        name: public_disclosure_frame(filename, pd.read_csv(ROOT / "tables" / filename))
    ...
    ws.title = "README"
    ws.sheet_view.showGridLines = False
    ws["A1"] = "Selective outcome observation study v3.2.4"
    ws["A1"].font = Font(size=16, bold=True, color=WHITE)
    ws["A1"].fill = PatternFill("solid", fgColor=NAVY)
    ws.merge_cells("A1:D1")
    readme = [
        ("Analysis status", "Scientific analysis complete"),
        ("Peer-review status", "Not peer reviewed"),
        ("Clinical deployment status", "Not evaluated; no clinical use is supported by this study"),
        ...
        (
            ...
            "Raw-rebuild equivalence is included as internal release evidence; it is not regenerated by the public replay because prior locked patient-level inputs are excluded",
        ),
        ("Disclosure control", "Positive aggregate cells below 5 are displayed as <5; percentages use the matching upper bound"),
    ]
    ...
    qa_rows = [
        ...
        ("Frozen internal raw-rebuild evidence: differing cells", int(rebuild["differing_cells"].sum())),
        ("Bootstrap replicates", 1000),
        ("MNAR bootstrap rows", 18000),
        ("Clinical deployment status", "Not evaluated for clinical use"),
        ("Peer-review status", "Not peer reviewed"),
    ]
    for row, values in enumerate(qa_rows, 1):
        for column, value in enumerate(values, 1):
            cell = qa_ws.cell(row, column, value)
            cell.alignment = Alignment(wrap_text=True, vertical="center")
    for cell in qa_ws[1]:
        cell.fill = PatternFill("solid", fgColor=NAVY)
        cell.font = Font(color=WHITE, bold=True)
    qa_ws.column_dimensions["A"].width = 52
    qa_ws.column_dimensions["B"].width = 54
